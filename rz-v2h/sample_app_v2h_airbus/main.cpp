...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        /*CHWをHWCに戻す*/
        float hcw_image[512*512*3];
        for(int i = 0;  i < 512*512; i++){
            ((float*)hcw_image)[i*3+2]  = ((float *)output_ptr)[i];
            ((float*)hcw_image)[i*3+1]  = ((float *)output_ptr)[i+512*512];
            ((float*)hcw_image)[i*3+0]  = ((float *)output_ptr)[i+512*512*2];
        }       
        cv::Mat outputimg(512, 512, CV_32FC3, hcw_image);
