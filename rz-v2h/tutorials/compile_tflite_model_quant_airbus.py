...
def pre_process_imagenet_pytorch(img, mean=[0.485, 0.456, 0.406], stdev=[0.229, 0.224, 0.225], dims=None, need_transpose=False):
    ...
    img = F.resize(img, 512, Image.BILINEAR)
...
if __name__ == "__main__":
    ...
    # 2. Load tflite model and set input shape.
    ...
    if input_dir is None:
        ...
        for i in range(len(input_list)):
            ...
            input_data = pre_process_imagenet_pytorch(image, mean, stdev, need_transpose=False)
    ...
    json, params, lib_path = drp.build(mod, params, "arm", drp_config, output_dir=opts["output_dir"], disable_concat = opts["disable_concat"], cpu_data_type=opts["cpu_data_type"])
